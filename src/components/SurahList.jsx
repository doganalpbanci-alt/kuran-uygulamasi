...
import { getLastReadMap } from "../lib/streak";
...
/**
 * Kartın solundaki rozet: mushaf sırasında sure numarası, nüzûl sırasında
 * iniş sırası. Kısmi bölümlerde (Âyetü'l-Kürsî) ait olduğu surenin
 * numarası gösteriliyor — liste zaten o surenin hemen ardında.
 */
function badgeFor(surah, order) {
  if (order === "revelation") return surah.revelation_order ?? null;
  if (order === "mushaf") return surah.audio_surah ?? surah.id;
  return null;
}

export default function SurahList({ surahs, order = null, onSelectSurah }) {
  ...
  // 114 sure listelenirken sure başına localStorage okumamak için
  // son okunma tarihleri tek seferde alınıyor.
  const [lastReadMap] = useState(getLastReadMap);
  ...
  return (
    ...
    <ul className="flex flex-col gap-3">
      {surahs.map((surah) => {
        const lastRead = lastReadMap[surah.id] ?? null;
        const badge = badgeFor(surah, order);
        ...
        return (
          <li key={surah.id} className="flex items-stretch gap-2">
            <button
              ...
              className="flex min-w-0 flex-1 items-center justify-between rounded-2xl border border-teal-600/15 bg-white/60 px-5 py-4 text-left shadow-sm transition hover:border-teal-600/40 dark:border-cream-200/15 dark:bg-white/5"
            >
              <div className="flex min-w-0 items-baseline gap-2">
                {badge != null && (
                  <span className="w-8 shrink-0 rounded-full bg-teal-600/10 px-2 py-0.5 text-center text-xs font-medium text-teal-700 dark:bg-white/10 dark:text-gold-500">
                    {badge}
                  </span>
                )}
              ...
              <span className="ml-3 shrink-0 basis-20 text-right text-xs text-teal-700 dark:text-gold-500">
